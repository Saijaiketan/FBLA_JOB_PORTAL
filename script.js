...
function displayDeletedApplications() {
    ...
    if (deletedAppsList) {
        ...
        // Populate the deleted list with applications if available
        deletedAppsList.innerHTML = deletedApplications.map((app) => `
...
function toggleDeletedApplications() {
    const deletedAppsList = document.getElementById('deleted-applications-list');
    if (deletedAppsList) {
        deletedAppsList.style.display = deletedAppsList.style.display === 'none' ? 'block' : 'none';
    }
}
...
function toggleJobApplications() {
    const applicationsDiv = document.getElementById('job-applications');
    if (applicationsDiv) {
        applicationsDiv.style.display = applicationsDiv.style.display === 'none' ? 'block' : 'none';
    }
}
